...
@section('contenido')
    <fieldset>
        <h1 class="align-text-center" class="legend">Agregar Usuario</h1>
        ...
        <form method="post" action="{{route('usuarios.store')}}">
            ...
            @error('correo_electronico')
               is-invalid
            @enderror
            "id="correo_electronico" name="correo_electronico" placeholder="Correo" value = "{{old('correo_electronico')}}">
                @error('correo_electronico')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
